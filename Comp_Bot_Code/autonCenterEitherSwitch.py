    ...
    def run(self):
        if self.side == 'center':
            if self.switchPosition == 'L':
                ...
                self.drive.autonMove(3, 0, speed = 1, distance = 40, setLiftPosition = 2)

                self.drive.autonMove(4, 1, turnAngle = 38, turnSpeed = .75, setLiftPosition = 2)

                self.drive.autonMove(5, 0, speed = .35, distance = 12, setLiftPosition = 2)

                self.drive.autonMove(6, 0, speed = -.75, distance = 35, setLiftPosition = 2, intakeMode = 3)

                self.drive.autonMove(7, 1, turnAngle = 50, turnSpeed = .65, setLiftPosition = 0)
                '''
                self.drive.autonMove(8, 0, speed = .4, distance = 45, setLiftPosition = 0, intakeMode = 1)

                self.drive.autonMove(9, 0, speed = -.5, distance = 40, setLiftPosition = 0)

                self.drive.autonMove(10, 1, turnAngle = -40, turnSpeed = .75, setLiftPosition = 2)
                ...
                self.drive.autonMove(3, 0, speed = 1, distance = 37, setLiftPosition = 2)

                self.drive.autonMove(4, 1, turnAngle = -37, turnSpeed = .84, setLiftPosition = 2)

                self.drive.autonMove(5, 0, speed = .8, distance = 10, setLiftPosition = 2, intakeMode = 3)
