    ...
    def run(self):
        if self.side == 'center':
            if self.switchPosition == 'L':
                ...
                self.drive.autonMove(4, 1, 0, 0, 38, .75, 2, 0)

                self.drive.autonMove(5, 0, speed = .8, distance = 7, setLiftPosition = 2)

                self.drive.autonMove(6, 0, speed = -.75, distance = 35, setLiftPosition = 2, intakeMode = 3)

                self.drive.autonMove(7, 1, turnSpeed = .65, turnAngle = 46, setLiftPosition = 0)

                self.drive.autonMove(8, 0, speed = .4, distance = 40, setLiftPosition = 0, intakeMode = 1)

                self.drive.autonMove(9, 0, speed = -.5, distance = 40)

                self.drive.autonMove(10, 1, turnSpeed = .75, turnAngle = -40, setLiftPosition = 2)
